...
export default function RowsPerPage({ table, pagination }) {
    ...
    const [selectedKey, setSelectedKey] = React.useState(Number(pagination.pageSize)); // Initialize with string for Select compatibility
    ...
    return (
        <Select onValueChange={handleSelectionChange} defaultValue={selectedKey}>
            <SelectTrigger className="w-[180px]">
                <SelectValue placeholder={`Rows per page: ${selectedKey}`} />
            </SelectTrigger>
            <SelectContent>
                {SIZE.map(num => (
                    <SelectItem key={num} value={num}>Rows per page: {num}</SelectItem>
                ))}
            </SelectContent>
        </Select>
    );
