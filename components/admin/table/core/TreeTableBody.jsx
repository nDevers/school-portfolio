...
export default function TreeTableBody({ table, extractedIds, handlePermissionId }) {
  return (
    <div>
        <table className='table-auto border-collapse w-full rounded-t-md overflow-hidden'>
            <thead>
                {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id} className="bg-violet-600 text-gray-200 text-nowrap">
            ...
            <tbody>
                {table.getRowModel().rows.map((row) => (
                    <tr key={row.id} className="border-t border-gray-200 hover:bg-blue-300/50 even:bg-gray-100 text-nowrap">
                        {row.getVisibleCells().map((cell) => (
                            <td key={cell.id} className="px-4 py-2 text-left flex items-center">
                                {/* <input 
                                    type="checkbox" 
                                    name="" 
                                    id={`checkbox-${cell.id}`} 
                                ...
                                <Checkbox 
                                    ...
                                    id={`checkbox-${cell.id}`} 
                                    value={cell.getContext().row.original.id}
                                    isSelected={extractedIds.some(({ menu_id }) => menu_id === cell.getContext().row.original.id)} 
                                    onChange={(e)=>handlePermissionId(e.target.value, e.target.checked)}
                                />
                                {flexRender(
