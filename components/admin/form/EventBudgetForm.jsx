...
export default function EventBudgetForm({ data }) {
    ...
    const validationSchema = Yup.object().shape({
        ...
        budget: Yup.number()
            ...
            .test('budget-limit', 'Budget exceeds the allowed Budget', function(value) {
        ...
        transactionType: Yup.string()
            ...
            .test('restrict-return', 'Return action is not allowed without a previous budget', function(value) {
    ...
    const { isLoading: eventDetailsLoading, data: eventDetails } = useQuery({
        ...
        queryFn: async () => {
            const data = await fetchData(apiConfig?.GET_EVENT_BY_ID + formik.values?.eventId)
            if (data) {
    ...
    return (
        <form onSubmit={formik.handleSubmit} className='w-full space-y-10'>
            <div className='grid md:grid-cols-3 gap-2 w-full'>
                <InputWrapper label="Event" error={formik.errors?.eventId} touched={formik.touched?.eventId}>
                    <ComboboxFormik
                        select="_id"
                        display="title"
                        name="eventId"
                ...
                <InputWrapper label="Budget" error={formik.errors?.budget} touched={formik.touched?.budget}>
                    <Input
                        name="budget"
                        type='number'
                        placeholder="Budget"
                ...
                <InputWrapper label="Transaction Type" error={formik.errors?.transactionType} touched={formik.touched?.transactionType}>
                    <ComboboxFormik
                        select="id"
                        display="type"
            ...
            {selectedEvent && (
                <Card>
                    <CardHeader>
                        <h2 className="text-lg font-semibold">Selected Event Details:</h2>
                        <p><strong>Event Name:</strong> {selectedEvent?.title}</p>
                        {selectedEvent?.previousBudget && (
                            <p><strong>Previous Budget:</strong> ${selectedEvent?.previousBudget}</p>
                        )}
                    </CardHeader>
                </Card>
            )}
