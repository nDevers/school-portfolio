...
export default function PaymentMethodForm({ data }) {
    ...
    const submit = async (values) => {
        if (data) {
            await updateData(apiConfig?.UPDATE_PAYMENT_METHOD + data?._id, values);
        } else {
    ...
    return (
        <form onSubmit={formik.handleSubmit} className='w-full space-y-10'>
            <div className='grid gap-2 w-full'>
                <InputWrapper label="Payment Method" error={formik.errors?.type} touched={formik.touched?.type}>
                    <Input
                        name="type"
