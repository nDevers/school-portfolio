...
import { clearField, handleImageChange, handleImageChangeForForm } from '@/util/formikHelpers'
...
export default function AcademicForm({ data, category }) {
    ...
    const submit = async (values) => {
        ...
        if (data) {
            await updateData(`${apiConfig?.UPDATE_ACADEMIC_BY_CATEGORY}${category}/${data?.id}`, formData);
        } else {
            await postData(apiConfig?.CREATE_ACADEMIC_BY_CATEGORY + category, formData);
        }
    }
        ...
        >
            {({ errors, touched, values, setFieldValue, resetForm }) => (
                <Form className="grid md:grid-cols-2 gap-4">
                    ...
                    <InputWrapper label="Badge" error={errors.badge} touched={touched.badge}>
                        <Field as={Input} name="badge" placeholder={`Badge of the ${category}`} />
                    </InputWrapper>

                    <InputWrapper label="Publish Date" error={errors.publishDate} touched={touched.publishDate}>
                        <Field as={Input} name="publishDate" type='date' />
                    </InputWrapper>

                    <InputWrapper label="Attachment" error={errors.file} touched={touched.file}>
                        <Field
                            as={Input}
                            type="file"
                            name="image"
                            accept="application/pdf, image/png, image/gif, image/jpeg, image/jpg"
                            onChange={(e) => handleImageChangeForForm(setFieldValue, 'file')(e)} />
                    </InputWrapper>

                    <InputWrapper label="Description" error={errors.description} touched={touched.description} className={'md:col-span-2'}>
